...
if __name__ == "__main__":
    ...
    i = img.unsqueeze(0).to(device)
    shape = i.shape[-2:]

    deform_layer = DeformationLayer(shape).to(device)

    with torch.no_grad():
        deform_layer.new_deformation(device)
        out = deform_layer.deform(i)
        out2 = deform_layer.back_deform(out)
        show(img.squeeze(), out.squeeze(), out2.squeeze(), deform_layer.deform(deform_layer.get_gird(device=device)))